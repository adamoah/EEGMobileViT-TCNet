...
class EEGMobileVit_TCN(nn.Module):
    def __init__(self):
        ...
        config = transformers.MobileViTConfig.from_pretrained(model_name)
        config.update({'num_channels': 768, 'image_size': (1, 14), 'patch_size': 1, 'conv_kernel_size ': 3})

        model = transformers.MobileViTForImageClassification.from_pretrained(model_name, config=config, ignore_mismatched_sizes=True)
